		...
		awsMachine := &infrav1.AWSMachine{
			...
			Spec: infrav1.AWSMachineSpec{
				AMI: infrav1.AWSResourceReference{
					ARN: pointer.StringPtr("aws:///test"),
				},
			},
		}
			...
			scope.MachineScopeParams{
				...
				Machine: &clusterv1.Machine{
					Spec: clusterv1.MachineSpec{
						...
						Version: pointer.StringPtr("1.16.0"),
					},
				},
	...
	Context("Reconciling an AWSMachine", func() {
		When("we can't reach amazon", func() {
			...
			It("should exit immediately on an error state", func() {
				er := string(capierrors.CreateMachineError)
				ms.AWSMachine.Status.ErrorMessage = &er
	...
	Context("deleting an AWSMachine", func() {
		...
		Context("Instance not shutting down yet", func() {
			...
			It("should return an error when the instance can't be terminated", func() {
				...
				_, err := reconciler.reconcileDelete(ms, cs)
				Expect(errors.Cause(err).Error()).To(ContainSubstring("can't reach AWS to terminate machine"))
			...
			When("instance can be shut down", func() {
				...
				When("there are network interfaces", func() {
					...
					It("should error when it can't retrieve security groups", func() {
						expected := "can't reach AWS to list security groups"
						ec2Svc.EXPECT().GetCoreSecurityGroups(gomock.Any()).Return(nil, errors.New(expected))

						_, err := reconciler.reconcileDelete(ms, cs)
						Expect(errors.Cause(err).Error()).To(ContainSubstring(expected))
					})

					It("should error when it can't detach a security group from an interface", func() {
						expected := "can't reach AWS to detach security group"
						ec2Svc.EXPECT().GetCoreSecurityGroups(gomock.Any()).Return([]string{"sg0", "sg1"}, nil)
						ec2Svc.EXPECT().DetachSecurityGroupsFromNetworkInterface(gomock.Any(), gomock.Any()).Return(errors.New(expected))
