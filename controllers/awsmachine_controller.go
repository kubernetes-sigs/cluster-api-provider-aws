...
func (r *AWSMachineReconciler) reconcileDelete(ctx context.Context, machineScope *scope.MachineScope, clusterScope cloud.ClusterScoper, ec2Scope scope.EC2Scope, elbScope scope.ELBScope, objectStoreScope scope.S3Scope) (ctrl.Result, error) {
	...
	case infrav1.InstanceStateTerminated:
		...
		if machineScope.AWSMachine.Status.AllocatedHostID != nil &&
			...
			machineScope.AWSMachine.Spec.DynamicHostAllocation.Release == infrav1.DedicatedHostReleaseStrategyOnMachineDeletion {
...
func getMaxHostReleaseRetries() int32 {
	return 5 // Configurable via environment variable or constant
}
...
func getInitialHostReleaseRetryDelay() time.Duration {
	return 30 * time.Second // Configurable via environment variable or constant
}
