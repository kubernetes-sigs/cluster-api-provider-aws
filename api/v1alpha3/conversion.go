...
func (r *AWSMachine) ConvertTo(dstRaw conversion.Hub) error {
	...
	restoreRootDeviceTags(&restored.Spec, &dst.Spec)
...
func (r *AWSMachineTemplate) ConvertTo(dstRaw conversion.Hub) error {
	...
	restoreRootDeviceTags(&restored.Spec.Template.Spec, &dst.Spec.Template.Spec)
...
// Manually restore the instance root device data
func restoreRootDeviceTags(restored, dst *v1alpha4.AWSMachineSpec) {
	if restored == nil {
		return
	}
	dst.RootDeviceID = restored.RootDeviceID
