...
type path struct {
	Get httpAPI `json:"get"`
	Put httpAPI `json:"put"`
	Post httpAPI `json:"post"`
	Patch httpAPI `json:"patch"`
	Delete httpAPI `json:"delete"`
}

type httpAPI struct {
	Responses map[string]requestResponse `json:"responses,omitempty"`
	RequestBody requestResponse `json:"requestBody,omitempty"`
}
...
type content struct {
	JSONData jsonData `json:"application/json"`
}

type jsonData struct {
	SchemaRef schemaRef `json:"schema"`
}
