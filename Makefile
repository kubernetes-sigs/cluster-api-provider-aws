...
.build/generate-go-apis: .build $(API_SRCS) $(CONTROLLER_GEN) $(DEFAULTER_GEN) $(CONVERSION_GEN)
	...
	$(CONVERSION_GEN) \
		...
		--extra-peer-dirs=sigs.k8s.io/cluster-api-provider-aws/api/v1alpha2 \
...
docker-build: docker-pull-prerequisites ## Build the docker image for controller-manager
	...
	docker build --build-arg ARCH=$(ARCH) --build-arg LDFLAGS="$(LDFLAGS)" . -t $(CORE_CONTROLLER_IMG_CHINA):$(TAG)
