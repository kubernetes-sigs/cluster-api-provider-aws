...
CONTROLLER_IMG ?= $(REGISTRY)/cluster-api-aws-controller
TAG ?= dev
ARCH ?= amd64
...
release:  ## Builds and push container images using the latest git tag for the commit.
	...
	REGISTRY=gcr.io/k8s-staging-cluster-api-aws TAG=$(RELEASE_TAG) \
		$(MAKE) docker-build-all docker-push-all
	# Set the manifest image to the production bucket.
	REGISTRY=us.gcr.io/k8s-artifacts-prod/cluster-api-aws TAG=$(RELEASE_TAG) \
		set-manifest-image
...
release-staging-latest: ## Builds and push container images to the staging bucket using "latest" tag.
	REGISTRY=gcr.io/k8s-staging-cluster-api-aws TAG=latest \
		$(MAKE) docker-build-all docker-push-all
