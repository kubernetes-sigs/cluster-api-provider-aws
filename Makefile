...
clusterctl: generate ## Build clusterctl binary.
	bazel build --workspace_status_command=./hack/status.sh //cmd/clusterctl $(BAZEL_ARGS)

.PHONY: clusterawsadm
clusterawsadm: dep-ensure ## Build clusterawsadm binary.
	bazel build --workspace_status_command=./hack/status.sh //cmd/clusterawsadm $(BAZEL_ARGS)
