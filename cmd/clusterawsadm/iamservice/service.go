...
// IAMAPI defines AWS SDK v2 methods used by iamservice package.
...
func (s *serviceImpl) CreateRole(ctx context.Context, resource go_cfn.Resource, tags map[string]string) error {
	...
	if len(res.ManagedPolicyArns) != 0 {
...
func (s *serviceImpl) CreatePolicy(ctx context.Context, resource go_cfn.Resource, tags map[string]string) error {
	...
	policy, err := s.IAM.CreatePolicy(ctx, &iam.CreatePolicyInput{
		Description:    &res.Description,
		...
		if errors.As(err, &apiErr) {
			...
			case EntityAlreadyExists:
				...
				for _, policy := range policies {
					if *policy.PolicyName == res.ManagedPolicyName && len(res.Roles) != 0 {
	...
	if len(res.Roles) != 0 {
		err = s.attachRolesToPolicy(ctx, policy.Policy, res.Roles)
		if err != nil {
			return err
		}
	}
