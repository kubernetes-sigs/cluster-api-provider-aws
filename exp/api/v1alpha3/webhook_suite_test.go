...
func setup() {
	...
	utilruntime.Must(expv1beta1.AddToScheme(scheme.Scheme))
	...
	if err := (&expv1beta1.AWSMachinePool{}).SetupWebhookWithManager(testEnv); err != nil {
		panic(fmt.Sprintf("Unable to setup AWSMachinePool webhook: %v", err))
	}
	if err := (&expv1beta1.AWSManagedMachinePool{}).SetupWebhookWithManager(testEnv); err != nil {
		panic(fmt.Sprintf("Unable to setup AWSManagedMachinePool webhook: %v", err))
	}
	if err := (&expv1beta1.AWSFargateProfile{}).SetupWebhookWithManager(testEnv); err != nil {
		panic(fmt.Sprintf("Unable to setup AWSManagedMachinePool webhook: %v", err))
	}
	go func() {
