...
func (*awsManagedMachinePoolTemplateWebhook) ValidateCreate(_ context.Context, obj runtime.Object) (admission.Warnings, error) {
	...
	if errs := validateScaling(&r.Spec.Template.Spec); errs != nil || len(errs) == 0 {
		allErrs = append(allErrs, errs...)
	}

	if errs := validateRemoteAccess(&r.Spec.Template.Spec); len(errs) > 0 {
		allErrs = append(allErrs, errs...)
	}

	if errs := validateNodegroupUpdateConfig(&r.Spec.Template.Spec); len(errs) > 0 {
		allErrs = append(allErrs, errs...)
	}

	if errs := validateLaunchTemplate(&r.Spec.Template.Spec); len(errs) > 0 {
		allErrs = append(allErrs, errs...)
	}
...
func (*awsManagedMachinePoolTemplateWebhook) ValidateUpdate(_ context.Context, oldObj, newObj runtime.Object) (admission.Warnings, error) {
	...
	var allErrs field.ErrorList
	allErrs = append(allErrs, validateAMPImmutable(&oldPool.Spec.Template.Spec, &r.Spec.Template.Spec)...)
	allErrs = append(allErrs, r.Spec.Template.Spec.AdditionalTags.Validate()...)
