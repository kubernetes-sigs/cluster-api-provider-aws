...
func TestAWSFargateProfileDefault(t *testing.T) {
	fargate := &expinfrav1.AWSFargateProfile{ObjectMeta: metav1.ObjectMeta{Name: "foo", Namespace: "default"},
		Spec: expinfrav1.FargateProfileSpec{
			ClusterName: "clustername",
		},
	}
	t.Run("for AWSFargateProfile", utildefaulting.DefaultValidateTest(context.Background(), fargate, &AWSFargateProfile{}))
	g := NewWithT(t)
	err := (&AWSFargateProfile{}).Default(context.Background(), fargate)
...
func TestAWSFargateProfileValidateRoleNameUpdate(t *testing.T) {
	...
	before := &expinfrav1.AWSFargateProfile{
	...
	defaultRoleNameUpdate.Spec.RoleName = expinfrav1.DefaultEKSFargateRole
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			warn, err := (&AWSFargateProfile{}).ValidateUpdate(context.Background(), tt.before.DeepCopy(), tt.fargateProfile)
			if tt.expectErr {
...
func TestAWSFargateProfileValidateCreate(t *testing.T) {
	...
	tests := []struct {
		name    string
		profile *expinfrav1.AWSFargateProfile
		wantErr bool
	}{
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			warn, err := (&AWSFargateProfile{}).ValidateCreate(context.Background(), tt.profile)
			if tt.wantErr {
				g.Expect(err).To(HaveOccurred())
			} else {
				g.Expect(err).To(Succeed())
