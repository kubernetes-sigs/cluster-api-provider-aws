...
func TestAWSMachinePoolDefault(t *testing.T) {
	m := &expinfrav1.AWSMachinePool{ObjectMeta: metav1.ObjectMeta{Name: "foo", Namespace: "default"}}
	t.Run("for AWSCluster", utildefaulting.DefaultValidateTest(context.Background(), m, &AWSMachinePool{}))
	err := (&AWSMachinePool{}).Default(context.Background(), m)
	g := NewWithT(t)
...
func TestAWSMachinePoolValidateCreate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			pool: &expinfrav1.AWSMachinePool{
				Spec: expinfrav1.AWSMachinePoolSpec{
					AWSLaunchTemplate: expinfrav1.AWSLaunchTemplate{AdditionalSecurityGroups: []infrav1.AWSResourceReference{{
						ID: aws.String("sg-1"),
			...
			pool: &expinfrav1.AWSMachinePool{
				Spec: expinfrav1.AWSMachinePoolSpec{
					MixedInstancesPolicy: &expinfrav1.MixedInstancesPolicy{
						Overrides: []expinfrav1.Overrides{{InstanceType: "t3.medium"}},
					},
					AWSLaunchTemplate: expinfrav1.AWSLaunchTemplate{
		...
		},
		{
			...
			pool: &expinfrav1.AWSMachinePool{
				Spec: expinfrav1.AWSMachinePoolSpec{
					RefreshPreferences: &expinfrav1.RefreshPreferences{MaxHealthyPercentage: aws.Int64(100)},
				},
			},
			...
			pool: &expinfrav1.AWSMachinePool{
				Spec: expinfrav1.AWSMachinePoolSpec{
					AWSLifecycleHooks: []expinfrav1.AWSLifecycleHook{
						{
							Name:                "the-hook",
							LifecycleTransition: expinfrav1.LifecycleHookTransitionInstanceTerminating,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			warn, err := (&AWSMachinePool{}).ValidateCreate(context.Background(), tt.pool)
...
func TestAWSMachinePoolValidateUpdate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			warn, err := (&AWSMachinePool{}).ValidateUpdate(context.Background(), tt.old.DeepCopy(), tt.new)
			if tt.wantErrToContain != nil {
				g.Expect(err).ToNot(BeNil())
				if err != nil {
					g.Expect(err.Error()).To(ContainSubstring(*tt.wantErrToContain))
