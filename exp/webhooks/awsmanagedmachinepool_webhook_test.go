...
var (
	...
	oldAmiType                    = expinfrav1.Al2x86_64
	newAmiType                    = expinfrav1.Al2x86_64GPU
	oldCapacityType               = expinfrav1.ManagedMachinePoolCapacityTypeOnDemand
	newCapacityType               = expinfrav1.ManagedMachinePoolCapacityTypeSpot
	oldAvailabilityZoneSubnetType = expinfrav1.AZSubnetTypePublic
	newAvailabilityZoneSubnetType = expinfrav1.AZSubnetTypePrivate
)

func TestAWSManagedMachinePoolDefault(t *testing.T) {
	g := NewWithT(t)

	fargate := &expinfrav1.AWSManagedMachinePool{ObjectMeta: metav1.ObjectMeta{Name: "foo", Namespace: "default"}}
	t.Run("for AWSManagedMachinePool", utildefaulting.DefaultValidateTest(context.Background(), fargate, &AWSManagedMachinePool{}))

	err := (&AWSManagedMachinePool{}).Default(context.Background(), fargate)
...
func TestAWSManagedMachinePoolValidateCreate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			warn, err := (&AWSManagedMachinePool{}).ValidateCreate(context.Background(), tt.pool)
			if tt.wantErr {
...
func TestAWSManagedMachinePoolValidateUpdate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			warn, err := (&AWSManagedMachinePool{}).ValidateUpdate(context.Background(), tt.old.DeepCopy(), tt.new)
			if tt.wantErr {
				g.Expect(err).To(HaveOccurred())
			} else {
				g.Expect(err).To(Succeed())
