...
func (r *RosaNetworkReconciler) reconcileNormal(ctx context.Context, rosaNetScope *scope.RosaNetworkScope) (res ctrl.Result, reterr error) {
	...
	if cfStack == nil { // The CF stack does not exist yet
		...
		if err != nil {
			// FIXME: update error conditions
			return ctrl.Result{}, fmt.Errorf("Failed to create CF stack: %w", err)
		} else { // The CF stack already exists
		}
	} else {
		rosaNetScope.Info(fmt.Sprintf("CF stack %s alredy exists", rosaNetScope.RosaNetwork.Spec.Name))

		resources, err := awsClient.DescribeCFStackResources(ctx, stackName)
		if err != nil {
			return ctrl.Result{}, fmt.Errorf("Error fetching CF stack resources: %w", err)
		}
