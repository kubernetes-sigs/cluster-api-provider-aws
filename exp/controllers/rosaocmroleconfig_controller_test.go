...
func TestROSAOCMRoleConfigIdentityNamespacePermissions(t *testing.T) {
	h := newTestHarness(t)

	// Setup OCM mocks
	h.setupOCMCurrentAccount()
	h.setupOCMPoliciesHandler()
	h.setupOCMLinkCheckProgressive()
	h.setupOCMLinkPost()
	...
	// Create AWSClusterControllerIdentity with allowedNamespaces restriction
		...
		Spec: infrav1.AWSClusterControllerIdentitySpec{
			AWSClusterIdentitySpec: infrav1.AWSClusterIdentitySpec{
				AllowedNamespaces: &infrav1.AllowedNamespaces{
					NamespaceList: []string{"test-namespace"},
				},
			},
		},
	}
	createObject(h.g, controllerIdentity, "")
	h.t.Cleanup(func() { cleanupObject(h.g, controllerIdentity) })
	...
	createObject(h.g, testNamespace, "")
	h.t.Cleanup(func() { cleanupObject(h.g, testNamespace) })

	// Create ROSAOCMRoleConfig with identityRef and identityNamespace
	h.ocmRoleConfig = &expinfrav1.ROSAOCMRoleConfig{
		ObjectMeta: metav1.ObjectMeta{
			Name: fmt.Sprintf("test-identity-ns-%s", h.testID),
		},
		Spec: expinfrav1.ROSAOCMRoleConfigSpec{
			RolePrefix: "test",
			Profile:    expinfrav1.ROSAOCMRoleProfileStandard,
			...
			IdentityNamespace: "test-namespace", // Should match allowedNamespaces
		},
	}
	createObject(h.g, h.ocmRoleConfig, "")
	h.t.Cleanup(func() { cleanupObject(h.g, h.ocmRoleConfig) })
	...
	// Test: Reconcile should succeed because identityNamespace matches allowedNamespaces
	req := ctrl.Request{NamespacedName: types.NamespacedName{Name: h.ocmRoleConfig.Name}}
	h.g.Eventually(func(g Gomega) {
		_, err := reconciler.Reconcile(h.testCtx, req)
		g.Expect(err).ToNot(HaveOccurred())
...
func TestROSAOCMRoleConfigIdentityNamespaceNotPermitted(t *testing.T) {
	h := newTestHarness(t)
	...
	controllerIdentity := &infrav1.AWSClusterControllerIdentity{
		...
		Spec: infrav1.AWSClusterControllerIdentitySpec{
			AWSClusterIdentitySpec: infrav1.AWSClusterIdentitySpec{
				AllowedNamespaces: &infrav1.AllowedNamespaces{
					NamespaceList: []string{"allowed-namespace"},
				},
			},
		},
	}
	...
	// Create ROSAOCMRoleConfig with identityRef and wrong identityNamespace
	h.ocmRoleConfig = &expinfrav1.ROSAOCMRoleConfig{
		ObjectMeta: metav1.ObjectMeta{
			Name: fmt.Sprintf("test-identity-ns-denied-%s", h.testID),
		},
		Spec: expinfrav1.ROSAOCMRoleConfigSpec{
			RolePrefix: "test",
			Profile:    expinfrav1.ROSAOCMRoleProfileStandard,
			...
			IdentityNamespace: "wrong-namespace", // Does NOT match allowedNamespaces
		},
	}
	createObject(h.g, h.ocmRoleConfig, "")
	h.t.Cleanup(func() { cleanupObject(h.g, h.ocmRoleConfig) })

	// No AWS client needed since this should fail at identity permission check
	reconciler := h.buildReconciler(nil)

	// Test: Reconcile should fail because identityNamespace does NOT match allowedNamespaces
	req := ctrl.Request{NamespacedName: types.NamespacedName{Name: h.ocmRoleConfig.Name}}
	h.g.Eventually(func(g Gomega) {
		_, err := reconciler.Reconcile(h.testCtx, req)
		g.Expect(err).To(HaveOccurred())
		g.Expect(err.Error()).To(ContainSubstring("is not permitted to use"))
