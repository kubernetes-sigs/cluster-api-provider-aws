...
func (r *AWSManagedMachinePoolReconciler) reconcileNormal(
	...
	if machinePoolScope.ManagedMachinePool.Spec.AWSLaunchTemplate != nil {
		...
		// Update the AWSManagedMachinePool annotation immediately after creating a new template
	...
	if err := createAWSMachinesIfNotExists(ctx, awsMachineList, machinePoolScope.MachinePool, asg, ec2Scope, r.Client, ec2svc); err != nil {
		machinePoolScope.ManagedMachinePool.Status.Ready = false
		conditions.MarkFalse(machinePoolScope.ManagedMachinePool, clusterv1.ReadyCondition, expinfrav1.AWSMachineCreationFailed, clusterv1.ConditionSeverityWarning, err.Error())
		return fmt.Errorf("failed to create missing awsmachines: %w", err)
	}

	if err := deleteOrphanedAWSMachines(ctx, awsMachineList, asg, ec2Scope, r.Client); err != nil {
		machinePoolScope.ManagedMachinePool.Status.Ready = false
		conditions.MarkFalse(machinePoolScope.ManagedMachinePool, clusterv1.ReadyCondition, expinfrav1.AWSMachineDeletionFailed, clusterv1.ConditionSeverityWarning, err.Error())
...
func (r *AWSManagedMachinePoolReconciler) reconcileDelete(
	...
	if err := reconcileDeleteAWSMachines(ctx, machinePoolScope.MachinePool, ec2Scope, r.Client); err != nil {
		return err
	}
