	...
	testcases := []struct {
		...
		},
		{
			...
			expect: func(m *mocks.MockEC2APIMockRecorder) {
				...
				m. // TODO: Restore these parameters, but with the tags as well
					RunInstancesWithContext(context.TODO(), gomock.Any()).
					DoAndReturn(func(_ context.Context, input *ec2.RunInstancesInput, _ ...interface{}) (*ec2.Reservation, error) {
						if input.CpuOptions == nil || input.CpuOptions.AmdSevSnp == nil || *input.CpuOptions.AmdSevSnp == "disabled" {
							t.Fatalf("expected AMD SEV-SNP to be enabled, but got %+v", input.CpuOptions)
						}
						return &ec2.Reservation{
							Instances: []*ec2.Instance{
								{
