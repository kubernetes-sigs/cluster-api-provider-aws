	...
	issuer := &v1certmanager.Issuer{
		ObjectMeta: metav1.ObjectMeta{
			Labels: map[string]string{
				clusterapiv1.ProviderLabelName: "infrastructure-aws",
			},
			Name:      issuerName,
...
func (s *Service) reconcileIdentityProvider(_ context.Context) error {
	...
	if arn == "" {
		...
		tags = append(tags, &iam.Tag{
			Key:   aws.String(v1beta1.ClusterAWSCloudProviderTagKey(s.scope.Name())),
			Value: aws.String(string(v1beta1.ResourceLifecycleOwned)),
		})
