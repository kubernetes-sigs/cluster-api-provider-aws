...
func (c *RosaClient) CreateUserIfNotExist(clusterID string, group, username string) (*clustersmgmtv1.User, error) {
	...
	user, err := clustersmgmtv1.NewUser().ID(username).Build()
	if err != nil {
...
func (c *RosaClient) CreateUser(clusterID string, group string, user *clustersmgmtv1.User) (*clustersmgmtv1.User, error) {
	response, err := c.ocm.ClustersMgmt().V1().
		Clusters().Cluster(clusterID).
		Groups().Group(group).
		Users().
