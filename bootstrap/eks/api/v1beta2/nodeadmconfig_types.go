...
type CustomUserDataOptions struct {
	...
	//   - {{.ContainerdBaseRuntimeSpec}}   - string of containerd base runtime spec YAML (if configured)
	//   - {{.FeatureGates}}                - map[Feature]bool of feature gates (if configured)
	//   - {{.PreNodeadmCommands}}          - []string of commands to run before nodeadm (if configured)
	//   - {{.Files}}                       - []File with resolved content (if configured)
	//   - {{.Users}}                       - []User to create (if configured)
	//   - {{.NTP}}                         - NTP configuration (if configured)
	//   - {{.DiskSetup}}                   - disk setup configuration (if configured)
	//   - {{.Mounts}}                      - []MountPoints to mount (if configured)
	//
